...
inline int geometricInt(double lambda) {
    if (lambda <= 0.0 || lambda > 1.0) {
        throw std::invalid_argument(
            "geometricInt: probability (lambda) must be in (0, 1], received: " + std::to_string(lambda)
        );
    }
    std::geometric_distribution<int> dist(lambda);
    return dist(threadLocalEngine());
}
