...
enum class DelayStyle { DS_UNIFORM, DS_POISSON, DS_ONE, DS_GEOMETRIC };

class ChannelProperties {
  public:
    DelayStyle delayStyle{DelayStyle::DS_UNIFORM};

    ChannelProperties(const nlohmann::json &params) { setParameters(params); }
    ...
    void setParameters(const nlohmann::json &params) {
        ...
        double minLambda = params.value("minLambda", 0.05);
        double maxLambda = params.value("maxLambda", 0.20);
        lambda = std::round(uniformReal(minLambda, maxLambda) * 100.0) / 100.0;
        std::string t = params.value("type", "UNIFORM");
        if (t == "UNIFORM")
            delayStyle = DelayStyle::DS_UNIFORM;
        else if (t == "POISSON")
            delayStyle = DelayStyle::DS_POISSON;
        else if (t == "ONE")
            delayStyle = DelayStyle::DS_ONE;
        else if (t == "GEOMETRIC")
            delayStyle = DelayStyle::DS_GEOMETRIC;
    }
    ...
    bool operator==(const ChannelProperties &other) const {
        return dropProbability == other.dropProbability &&
               ...
               maxMsgsRec == other.maxMsgsRec && size == other.size &&
               avgDelay == other.avgDelay && minDelay == other.minDelay &&
               maxDelay == other.maxDelay && delayStyle == other.delayStyle &&
               lambda == other.lambda;
    }
    ...
    struct Hash {
        size_t operator()(const ChannelProperties *props) const {
            ...
            h ^= std::hash<double>{}(props->getLambda());
            return h;
        }
    };
...
class ChannelPropertiesFactory {
  ...
  private:
    ...
    ChannelPropertiesFactory &
    operator=(const ChannelPropertiesFactory &) = delete;

    std::unordered_set<ChannelProperties *, ChannelProperties::Hash>
        _propertiesCache;
};
...
class Channel : public std::enable_shared_from_this<Channel> {
  private:
    ...
    int _throughputLeft{
        INT_MAX
    }; // throughputLeft, if you want a limited number of sends to reduce the
       // maximum size of the channel
    ...
    bool canSend() const {
        return (
            _throughputLeft != 0 &&
            (_properties->getSize() > _packetQueue.size())
        );
    }
  ...
  public:
    ...
    Channel(
        interfaceId targetId, interfaceId targetInternalId,
