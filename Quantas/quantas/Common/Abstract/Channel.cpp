...
Channel::Channel(
    interfaceId targetId, interfaceId targetInternalId, interfaceId sourceId,
    interfaceId sourceInternalId, const nlohmann::json &channelParams
)
    : _targetId(targetId), _targetInternalId(targetInternalId),
      _sourceId(sourceId), _sourceInternalId(sourceInternalId) {
...
void Channel::setParameters(const nlohmann::json &params) {
    ...
    _throughputLeft =
        _properties->getMaxMsgsRec() *
        (RoundManager::lastRound() - RoundManager::currentRound());
}
...
int Channel::computeRandomDelay() const {
    ...
    case DelayStyle::DS_UNIFORM:
        delay =
            uniformInt(_properties->getMinDelay(), _properties->getMaxDelay());
        break;
    case DelayStyle::DS_POISSON:
        delay = poissonInt(_properties->getAvgDelay());
        delay = std::clamp(
            delay, _properties->getMinDelay(), _properties->getMaxDelay()
        );
        break;
    case DelayStyle::DS_ONE:
        delay = 1;
        break;
    case DelayStyle::DS_GEOMETRIC:
        delay = geometricInt(_properties->getLambda());
    }
    return delay;
}
...
void Channel::shuffleChannel() {
    ...
    if (_packetQueue.size() > 1 &&
        trueWithProbability(_properties->getReorderProbability())) {
        std::shuffle(
            _packetQueue.begin(), _packetQueue.end(), threadLocalEngine()
        );
    }
}
