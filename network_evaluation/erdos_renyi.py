...
def generate_networks(n, k, edge_prob, attempts: int = 10000, progress_counter=None, progress_lock=None, total_attempts=None, stop_flag=None):
    ...
    while attempt <= attempts:
        ...
        """ if is_k_conncted and is_k_level_ordering:
            save_network(pca_network, DIR_PATH / f"pca_{n}_{k}_{edge_prob:.2f}", f"pca_network{attempt}")
            save_network(dolev_network, DIR_PATH / f"dolev_{n}_{k}_{edge_prob:.2f}", f"dolev_network{attempt}") """
...
def erdos_renyi_plot():
    results = "results/erdos_renyi.csv"
    if not os.path.exists(results):
        print(f"No results file found at {results}. Please run the erdos_renyi script first.")
        return
    ...
    plt.xlabel('edge probability (p)', fontsize=FT)
    ...
    plt.title('Erdos Renyi Comparison for a network of n=100 nodes\n on 1000 attempts', fontsize=FT)
    ...
    plt.savefig("img/erdos_renyi.png", dpi=600)
    plt.savefig("img/erdos_renyi.pdf", dpi=600)
    ...
    plt.ylim(top=110)
    plt.yticks([0, 20, 40, 60, 80, 100], fontsize=FT-4)
    plt.title('[DOLEV] Erdos Renyi Comparison for a network of n=100 nodes\n on 1000 attempts', fontsize=FT)
    plt.legend(fontsize=FT-3)
    plt.savefig("img/erdos_renyi_dolev.png", dpi=600)
    plt.savefig("img/erdos_renyi_dolev.pdf", dpi=600)
    ...
    plt.ylim(top=110)
    plt.yticks([0, 20, 40, 60, 80, 100], fontsize=FT-4)
    plt.title('[PCA] Erdos Renyi Comparison for a network of n=100 nodes\n on 1000 attempts', fontsize=FT)
    plt.legend(fontsize=FT-3)
    plt.savefig("img/erdos_renyi_pca.png", dpi=600)
    plt.savefig("img/erdos_renyi_pca.pdf", dpi=600)
...
if __name__ == "__main__":
    ...
    for k in range(min_k, max_k + 1):
        ...
        print(f"[erdos_renyi] Remaining edge probabilities for k={k}: {edges_prob}", flush=True)
        if edges_prob == []:
            print(f"[erdos_renyi] All edge probabilities have been processed for k={k}. No remaining tries.", flush=True)
            continue
