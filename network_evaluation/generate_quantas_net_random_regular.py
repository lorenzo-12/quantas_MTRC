...
while True:
    ...
    x = generate_networks(n=50, k=k_val, d=dd, attempts=1000, net_to_generate=10, combo="both")
    if x == 10:
        break 
    
    # delete folder and try again 
    clear_directory(DIR_PATH / f"both_{50}_{k_val}_{dd}")
    os.rmdir(DIR_PATH / f"both_{50}_{k_val}_{dd}")
    dd += 1
...
while True:
    ...
    x = generate_networks(n=50, k=k_val, d=dd, attempts=1000, net_to_generate=10, combo="dolev")
    if x == 10:
        break 
...
def get_output_file(n, k, t, topology_name="both_networks0"):
    return f"results/random_regular_{n}_{k}_{t}_{topology_name}.json"
...
for t, ts, tl in combinations:
    ...
    with open(f"quantas_topologies/experiments/random_regular_n{n}_k{k}_t{t}.json", "w") as f:
...
k5 = ""
k7 = ""
for file in dir.glob("random_regular_n*_k*_t*.json"):
