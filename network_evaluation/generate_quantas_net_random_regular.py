...
while True:
    ...
    x = generate_networks(n=100, k=k_val, d=dd, attempts=1000, net_to_generate=10, combo="both")
    if x == 10:
        break 
    
    # delete folder and try again 
    clear_directory(DIR_PATH / f"both_{100}_{k_val}_{dd}")
    os.rmdir(DIR_PATH / f"both_{100}_{k_val}_{dd}")
    dd += 1
...
while True:
    ...
    x = generate_networks(n=100, k=k_val, d=dd, attempts=1000, net_to_generate=10, combo="dolev")
    if x == 10:
        break 
...
def get_output_file(n, k, t):
    return f"results/random_regular/{n}_{k}_{t}.json"
